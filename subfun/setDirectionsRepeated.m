...
function cfg = setDirectionsRepeated(cfg)
    ...
    static_directions=[-1 -2 -1 -2 -1 -2 -1 -2 -1 -2 -1 -2 ];
...
%                                1, NB_REPEATS_BASE_VECTOR);
    ...
    %%%create the direction matrix with targets for 24 events
%     for j=1:size(cfg.design.soundTargets,2)/2
%         cfg.design.directions(:,2*j)=cfg.design.directions(:,2*j-1).*(cfg.design.soundTargets(:,2*j-1).*cfg.design.soundTargets(:,2*j))+cfg.design.directions(:,2*j).*(ones(size(cfg.design.soundTargets,1),1)-cfg.design.soundTargets(:,2*j-1).*cfg.design.soundTargets(:,2*j));
% 
%     end
